...
def confidence_diff_boxplot(df, model_names, pixel_counts):
    ...
    - model_names: List of model names (e.g., ['VGG16', 'AllConv']).
...
def outliers_by_class_label(outliers_df):
    """
    Plots the class distribution for each model of outliers in the given DataFrame.
...
def summary_tables(df, targeted = False):
  """
    ...
    - 'target_class': The intended target class for the attack if targrted = False.
